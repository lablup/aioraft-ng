...
class GrpcRaftServer(RaftServer, raft_pb2_grpc.RaftServiceServicer):
    ...
    def AppendEntries(
        ...
        logging.info(f'[AppendEntries] term={request.term} leader={request.leader_id}')
    ...
    def RequestVote(
        ...
        logging.info(f'[RequestVote] term={request.term} candidate={request.candidate_id}')
    ...
    @staticmethod
    async def run(servicer: raft_pb2_grpc.RaftServiceServicer, port: int = 50051):
        ...
        _cleanup_coroutines = [server_graceful_shutdown()]
        """
