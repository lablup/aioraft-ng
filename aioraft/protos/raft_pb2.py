...
class InstallSnapshotRequest:
    ...
    def FromString(cls, raw: bytes) -> "InstallSnapshotRequest":
        offset = 0
        term = _struct.unpack_from("!Q", raw, offset)[0]
        offset += 8
        leader_len = _struct.unpack_from("!I", raw, offset)[0]
        offset += 4
        leader_id = raw[offset : offset + leader_len].decode("utf-8")
        offset += leader_len
        last_included_index = _struct.unpack_from("!Q", raw, offset)[0]
        offset += 8
        last_included_term = _struct.unpack_from("!Q", raw, offset)[0]
        offset += 8
        data = raw[offset:]
        return cls(
            term=term,
