...
class Raft(aobject, AbstractRaftProtocol):
        ...
        while True:
            match self._state:
                ...
                case RaftState.LEADER:
                    ...
                    await self.establish_authority()
    ...
    async def _change_state(self, next_state: RaftState) -> None:
        if self._state is next_state:
            return
        log.debug(f"[{self._id[-5:]}] change_state(): {self._state} -> {next_state}")
    ...
    async def replicate(self, entries: Iterable[raft_pb2.Log]) -> bool:
        ...
        if entries := tuple(entries):
            if prev_log := await self._log.precede(entries[0].index):
    ...
    async def send_heartbeat_message(self) -> bool:
        return await self.replicate(entries=())

    def has_leadership(self) -> bool:
        return self._state is RaftState.LEADER

    async def establish_authority(self) -> None:
        log.info('Upon becoming leader, append "no-op".')
        count = await self._log.count()
        no_op_entry = raft_pb2.Log(
            index=count + 1,
            term=self.current_term,
            command=None,
        )
        await self._log.append(entries=(no_op_entry,))
        ...
        *,
        ...
        if leader_commit > self._commit_index:
            last_new_entry_index = (
                new_entries[-1].index if new_entries else self._commit_index
            )
            self._commit_index = min(leader_commit, last_new_entry_index)
        return (self.current_term, True)
